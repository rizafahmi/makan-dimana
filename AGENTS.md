...
* `src/lib/db.ts`, `src/lib/relay.ts` and `src/lib/share.ts` are server-only. `src/scripts/` is client-only: `app.ts`, `idb.ts` and `sync.ts` never run on the server. The rest of `src/lib` - `id.ts`, `merge.ts`, `retry.ts`, `session.ts`, `store.ts`, `time.ts`, `validate.ts` - is isomorphic. Client code must never import a server-only module.
